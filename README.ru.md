...
Инструмент для измерения пропускной способности и стабильности локальной сети. Измеряет задержку, джиттер, пропускную способность и битрейт в реальном времени между двумя устройствами в одной LAN — без серверов интернет-спидтестов, без телеметрии, без установки приложения на второе устройство.
...
- [Почему не обычный спидтест?](#почему-не-обычный-спидтест)
...
Запускает небольшой HTTP-сервер на вашем Windows-ПК. Другое устройство открывает страницу в браузере и выполняет полную последовательность тестов:
...
При запросе разрешите доступ в **частных сетях**.

Если страница не открывается с другого устройства:

- Убедитесь, что оба устройства находятся в одной подсети.
- Разрешите `speedband.exe` или `go.exe` в частных сетях в брандмауэре Windows.
...
- **Сохранить на сервер** — отправляет отчёт на ПК; сохраняет `.log`-файл в папке `reports/` в рабочем каталоге сервера.
...
## Почему не обычный спидтест?

Интернет-спидтест измеряет путь до внешнего сервера. Локальный стриминг проходит между двумя устройствами внутри LAN. Высокий результат в интернете не доказывает, что роутер, диапазон Wi-Fi или локальный канал выдержат реальный битрейт без джиттера или зависаний.

Для эталонного измерения LAN мощнее `iperf3` — но он требует установки клиента на оба устройства. SpeedBand требует только браузер на втором устройстве.

---

## Ограничения

- Второе устройство только через браузер; сырые UDP-сокеты недоступны.
- Односторонняя задержка неточна без синхронизации часов.
- WebSocket работает поверх TCP; потеря пакетов определяется по пробелам в последовательностях и зависаниям, а не по сырым UDP-потерям.
- Результат — оценочный диапазон битрейта, а не точная истина.
